...
Key differences in behavior here all 
...
def preprocess(config):
    ...
    This is broken into three parts: a first step which loads the
    data, applies most but not all of the usual initial filters, and
    converts to relative abundance, performs phylogenetic aggregation,
    log-transforms (if requested), etc., and performs phylogenetic aggregation; 
    a second step which transforms the `outer' problem to the effective problem 
    using the event data; and a third step which performs temporal resolution filtering.

    """
    data = preprocess_stepA(config)
    data = event_transform(config, data)
    data = preprocess_stepB(config, data)
    return data

def event_transform(config, data):
    """ Transform the true, time-dependent problem into an effective static outcome problem.

    Returns a new Dataset object.

    Attributes of the new Dataset that must be updated:
    subject_IDs, X, T, y, subject_data, n_subjects
    experiment_end, experiment_start
    additional_covariate_matrix (by calling generate_additional_covariate_matrix once 
    everything else is done)
    _primitive_result_cache (defensively reset to {})
    ...
    # Potentially some issues with floating point to integer equality checking
    # arise here but testing suggests it should be okay for the typical values
    ...
    # Given this, now set up the new dataset.
    n_subjects = len(pseudosubjects)
    subject_IDs = list(pseudosubjects.new_ID.values)
    
    subject_data = pd.DataFrame(columns=data.subject_data.columns)
    X = []
    T = []
    ...
    effective.y = pseudosubjects.y.values
    effective.experiment_start = 0.
    effective.experiment_end = lookback
    effective.X = X
    effective.T = T
    effective.subject_data = subject_data
...
def preprocess_stepA(config):
    ...
    # Skipped here, see preprocess_stepB
